...
# Resolved through _targetdir.ps1, never a hardcoded dev-machine path - a stale literal here
# silently keeps the format count frozen at whatever it last was on a different machine.
$st2k = Join-Path (& (Join-Path $PSScriptRoot '_targetdir.ps1')) 'release\st2k.exe'
if (Test-Path $st2k) {
