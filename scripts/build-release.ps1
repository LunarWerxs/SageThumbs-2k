...
if ($Portable) {
    ...
    Set-Content -LiteralPath "$portableStage\PORTABLE.txt" -Encoding utf8 -Value @(
        ...
        '  registered, so there is no way around that. The welcome window that opens the'
        '  first time you run SageThumbs2K.exe offers this as its first choice, so you do'
        '  not have to come looking. You can also turn them on any time with:'
        ''
        '      st2k register'
        ''
        '  or the button in Settings > Advanced. It writes to your own user account only,'
        '  needs no administrator rights, and touches nothing machine-wide. Undo it with:'
        ''
        '      st2k register --off'
        ''
