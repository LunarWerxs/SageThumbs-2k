...
if ($py -and (Test-Path $xcfGen)) {
    ...
    Set-Content -Path "$OutDir\_expected-colors.txt" -Encoding ascii -Value @(
        '# <sample><TAB><r,g,b> — the colour a CORRECT flatten produces (the top layer).',
        '# Generated by make-xcf-fixture.py; see build-corpus.ps1 section 9z.',
    ...
    Write-Host "[corpus] big layered .xcf samples + _expected-colors.txt written"
} else {
