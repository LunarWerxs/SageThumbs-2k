...
/// Put UTF-16 `text` on the clipboard as CF_UNICODETEXT (same ownership dance as
/// `verbs::copy_to_clipboard`).
unsafe fn copy_text_to_clipboard(text: &str) -> Result<()> {
    let wide: Vec<u16> = text.encode_utf16().chain(std::iter::once(0)).collect();
    let hmem = GlobalAlloc(GMEM_MOVEABLE, wide.len() * 2).map_err(|_| Error::from(E_FAIL))?;
