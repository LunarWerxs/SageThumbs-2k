...
fn save_png(path: &Path, w: u32, h: u32) -> String {
    image::DynamicImage::ImageRgb8(image::RgbImage::from_pixel(w, h, image::Rgb([40, 90, 200])))
        .save(path)
        .unwrap();
...
fn pdf_and_cbz_refuse_a_foreign_extension_and_an_output_that_is_an_input() {
    ...
    for (verb, name) in verbs {
        let err = verb(&png, std::slice::from_ref(&png), CombineOpts::default()).expect_err(name);
