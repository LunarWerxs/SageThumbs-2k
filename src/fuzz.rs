...
fn is_bookkeeping_seed(c: &SeedCandidate) -> bool {
    let name = c.name();
...
fn select_corpus_seeds(candidates: &[SeedCandidate], budget: usize) -> Vec<&SeedCandidate> {
...
/// cap even applied).
fn read_seed_bounded<R: std::io::Read>(src: R, cap: usize) -> Option<Vec<u8>> {
    use std::io::Read as _;
    let mut buf = Vec::new();
...
fn scan_corpus_candidates(roots: &[PathBuf]) -> Vec<SeedCandidate> {
...
fn load_selected_seeds(selected: &[&SeedCandidate], cap: usize) -> Vec<(String, Vec<u8>)> {
...
fn fuzz_seeds_over_header_parsers(
    seeds: &[(String, Vec<u8>)],
    targets: &[Target],
...
/// [`parsers_survive_mutation_of_corpus_samples`] to keep that test's own complexity below the
/// repo's per-function gate (2026-09-05 audit, F23).
fn fuzz_seeds_over_decode_cascade(
    seeds: &[(String, Vec<u8>)],
    overall: &std::time::Instant,
) -> Vec<String> {
...
/// back.
#[cfg(test)]
mod corpus_seed_selection_tests {
    use super::*;
