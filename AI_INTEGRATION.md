...
> [FEATURES.md](FEATURES.md) §6a and [ROADMAP.md](ROADMAP.md).
>
> **Phase 1 — `st2k.exe`** is built and installed: a console tool with verbs
> `thumbnail / convert / rotate / strip / ocr / pdf / info [--json] / formats
...
|---|---|---|
| Decode **179 formats** → PNG (RAW, HEIC, PSD, ebook/comic covers, PDF page 1, SVG, …) | image crate + WIC + bundled ImageMagick + resvg | `decode::decode_full` |
...
thumbnail <in> <out.png> [--size 256] # render any of the 179 types to PNG (default 256px)
...
- 8 tools = the 8 CLI verbs, each with a JSON-Schema, each calling `cli::*`. Tool
  failures return a result with `isError: true`; protocol faults use JSON-RPC errors.
