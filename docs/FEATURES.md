...
> rebuilds the abandoned SageThumbs (Explorer thumbnails for 322 file types plus
...
**322 registered extensions, in six categories** (also how the Options list is
grouped):

| Category | Examples | How |
|---|---|---|
| **Image** (193) | png, jpg, gif, bmp, tiff, webp, heic/heif/**heics/heifs/hif**, avif, psd, **psp/pspimage + pspbrush/pspframe/psptube/pspshape/pspselection/pspmask** (the Paint Shop Pro family), **iff/ilbm/lbm** (Amiga ILBM), **c4d** (Cinema 4D preview), **cdr/cdt/cmx** (CorelDRAW DISP preview), tga, dds, exr, ico, **icns** (Apple), **jxr/wdp/hdp/wmp** (JPEG XR / HD Photo), jp2/**jpf/jpx**, hdr/**rgbe/xyze**, svg/svgz, **wmf/emf/emz/wmz** (metafiles), **sketch/procreate/skp/3dm/dwg/max/c4d/xd/cdr/cdt** (design/CAD/3D), **blend/.blend1–32** (Blender + auto-saves), **ai** (Illustrator), **eps** (DOS-EPS preview), **f3d** (Autodesk Fusion 360), … | image crate / WIC / ImageMagick / resvg (SVG) |
...
*Counts sum to **322** (canonical source: `formats::FORMATS.len()`; `st2k formats` prints
...
capabilities (322-format decode incl. RAW/HEIC/ebook covers, ImageMagick, WIC, the
...
1. ✅ **CLI shipped** as a standalone **`st2k.exe`** (console subsystem): verbs
   ...
   (combine), `thumbnail` (render any of the 322 types to PNG), **`batch`** (bulk
...
2. ✅ **MCP server mode** (`st2k --mcp`, stdio JSON-RPC 2.0): exposes **10** MCP
   ...
   core verbs plus **`view`** (which decodes any of the 322 formats to a PNG **image
