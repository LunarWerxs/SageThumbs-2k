...
- **System-wide eyedropper / color picker** _(free)_ - A screen-wide color picker with a 10x magnifier loupe: click or press Space to copy the pixel under the cursor as hex/rgb()/hsl()/hsv() (Tab switches format), keeping the last 10 picks a keypress away. - `crates/screenshot/src/eyedropper.rs`
- **Screenshot capture + annotate** _(free)_ - A capture hotkey opens a region editor (draw/annotate, snap-to-45°, live width x height readout) with copy/save shortcuts, plus an optional quick-save hotkey that grabs the whole screen straight to clipboard + a timestamped PNG. - `crates/screenshot/src/screenshot/overlay.rs`
...
- **Auto-updater** _(free)_ - Checks GitHub for a newer release on a schedule (or on demand), and on approval downloads, integrity-checks and installs the update in the background with a quiet tray notification when done. - `crates/appkit/src/update.rs`
- **Commercial licensing (seat keys)** _(paid)_ - Free for personal use under PolyForm Noncommercial; a business copy gets a 7-day evaluation with every feature, then 3 days of reminders, then the shell stops serving thumbnails, previews, the Details pane and the menu until a seat key (esk_...) is redeemed under Settings ▸ Licence; a redeemed key is checked in the background and tolerant of about a week offline, and a copy that once held one is never stopped by silence, only by a revocation. A stored key can renew another 12 months of updates from a pre-filled checkout link, and a refused redemption (key already bound to another machine) now points the buyer at the Connections seat portal to re-bind it themselves instead of a dead end. - `crates/base/src/licence_state.rs`, `crates/appkit/src/license.rs`
