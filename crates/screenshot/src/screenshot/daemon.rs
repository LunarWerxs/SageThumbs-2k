...
use st2k_appkit::win::{app_icon, wide};
...
fn start_worker() {
    ...
    std::thread::spawn(move || {
        for work in rx {
            unsafe {
                match work {
                    DaemonWork::PreviewToggle => st2k_preview::preview::request_toggle(),
                    DaemonWork::PreviewClose => st2k_preview::preview::request_close(),
...
fn spawn(arg: Option<&str>) {
    ...
    let _ = match arg {
        Some(a) => st2k_appkit::win::spawn_self(&[a]),
        None => st2k_appkit::win::spawn_self(&[]),
    };
}

pub unsafe fn run_daemon(hinst: HINSTANCE) {
    ...
    let (lock, last_err) =
        st2k_appkit::win::create_mutex_user_only(true, w!("SageThumbs2K.ShotDaemon.Single"));
...
unsafe fn kick_licence_tick(hwnd: HWND) {
    ...
    std::thread::spawn(move || {
        if st2k_appkit::license::background_tick().is_some() {
            unsafe {
                let _ = PostMessageW(
                    Some(HWND(hwnd_raw as *mut core::ffi::c_void)),
                    WM_LICENCE_DUE,
...
unsafe fn on_licence_due(hwnd: HWND) {
    ...
    set_balloon_text(
        &mut nid.szInfoTitle,
        st2k_appkit::win::t("licence_popup_title"),
    );
    set_balloon_text(
        &mut nid.szInfo,
        &format!(
            "{} {}",
            st2k_appkit::license::licence_reminder_body(&snap),
            st2k_appkit::win::t("licence_toast_enter_key")
        ),
    );
    let _ = Shell_NotifyIconW(NIM_MODIFY, &nid);
    st2k_appkit::license::note_nag_shown(snap.now_unix);
}

/// Open Settings on the Licence page (the licence balloon's click target).
fn open_licence_settings() {
    let _ = st2k_appkit::win::spawn_self(&["--tab", "nav_licence"]);
}
