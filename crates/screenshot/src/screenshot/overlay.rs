...
use st2k_appkit::win::{app_icon, gui_font, wide};
...
pub unsafe fn run_capture(hinst: HINSTANCE) {
...
pub unsafe fn run_capture_ocr(hinst: HINSTANCE) {
...
pub unsafe fn run_capture_automation(hinst: HINSTANCE) {
...
unsafe fn claim_single_overlay_slot(name: PCWSTR) -> windows::core::Result<HANDLE> {
    let (lock, last_err) = st2k_appkit::win::create_mutex_user_only(true, name);
    let lock = lock?;
