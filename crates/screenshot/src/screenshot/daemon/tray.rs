...
pub(super) unsafe fn tray_data(hwnd: HWND, with_payload: bool) -> NOTIFYICONDATAW {
    ...
    if with_payload {
        ...
        if super::super::enable::autostart_missing_while_wanted() {
            tip_text.push(' ');
            tip_text.push_str(st2k_appkit::win::t("shot_tray_no_autostart"));
        }
...
pub(super) unsafe fn show_tray_menu(hwnd: HWND) {
    ...
    let (cap, ocr, ups, set, hide, quit) = (
        wide(st2k_appkit::win::t("tray_capture")),
        wide(st2k_appkit::win::t("tray_ocr")),
        wide(st2k_appkit::win::t("tray_uploads")),
        wide(st2k_appkit::win::t("tray_settings")),
        wide(st2k_appkit::win::t("tray_hide")),
        wide(st2k_appkit::win::t("tray_quit")),
    );
    ...
    st2k_appkit::win::force_foreground(hwnd);
...
pub(super) unsafe fn show_elevated_warning(hwnd: HWND, kind: &str) {
    ...
    set_balloon_text(
        &mut nid.szInfoTitle,
        &format!("{kind}: {}", st2k_appkit::win::t("admin_warn_title")),
    );
