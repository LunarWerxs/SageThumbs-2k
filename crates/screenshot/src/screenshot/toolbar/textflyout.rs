...
pub(crate) fn checkbox_label(checked: bool, name_key: &str) -> String {
    ...
    format!("{mark}  {}", st2k_appkit::win::t(name_key))
}
...
pub(crate) unsafe fn draw_text_flyout(
    ...
    for (it, r) in items {
        match it {
            ...
            TextItem::More => draw_btn(hdc, *r, st2k_appkit::win::t("shot_text_more_fonts")),
        }
    }
...
mod tests {
    ...
    fn the_open_dropdown_adds_one_row_per_preset_font_and_grows_the_panel() {
        ...
        let inset = st2k_appkit::win::dpi_scale_dpi(2, 96);
        let drop_h = nf * st2k_appkit::win::dpi_scale_dpi(TF_OPT, 96) + inset * 2;
        assert_eq!(
    ...
    fn the_whole_layout_scales_with_the_dpi() {
        ...
        assert_eq!(
            p96.right - p96.left,
            st2k_appkit::win::dpi_scale_dpi(TF_PW, 96)
        );
    ...
    fn checkbox_label_marks_the_state_and_reads_the_localized_name() {
        ...
        let name = st2k_appkit::win::t("shot_text_bold");
