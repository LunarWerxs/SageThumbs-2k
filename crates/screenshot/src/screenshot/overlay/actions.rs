...
pub(super) unsafe fn compose_and_spawn(s: &Shot, mode: &str) -> bool {
    ...
    let spawned = st2k_appkit::win::spawn_self(&[mode, &path]);
    if !spawned {
...
fn warn_save_failed(hwnd: HWND, dir: &str) {
    let m = wide(&st2k_appkit::win::t("shot_save_failed").replace("{dir}", dir));
...
pub(super) unsafe fn close_if_handed_off(hwnd: HWND, handed_off: bool) -> bool {
    if handed_off {
        let _ = DestroyWindow(hwnd);
    }
    handed_off
