...
pub(super) unsafe fn sample_pixel(s: &mut Shot, p: POINT) {
    ...
    if let Some(state) = s.automation.as_mut() {
        ...
        let _ = st2k_appkit::win::set_clipboard_text(&format!("#{r:02X}{g:02X}{b:02X}"));
    }
...
pub(super) unsafe fn loupe_rect(s: &Shot, cx: i32, cy: i32) -> RECT {
    ...
    let mag = st2k_appkit::win::dpi_scale_dpi(LOUPE_MAG, dpi);
    let lbl = st2k_appkit::win::dpi_scale_dpi(LOUPE_LBL, dpi);
    let gap = st2k_appkit::win::dpi_scale_dpi(18, dpi);
...
pub(super) unsafe fn draw_loupe(
    ...
    let lbl = st2k_appkit::win::dpi_scale_dpi(LOUPE_LBL, dpi);
    let gap = st2k_appkit::win::dpi_scale_dpi(18, dpi);
    let pad = st2k_appkit::win::dpi_scale_dpi(5, dpi);
    let swsz = st2k_appkit::win::dpi_scale_dpi(16, dpi);
