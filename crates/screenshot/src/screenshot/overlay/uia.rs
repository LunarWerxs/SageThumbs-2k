...
mod providers;
use providers::*;
use st2k_appkit::uia::{hwnd_of, no_element};
...
/// [`on_ui_thread`] and `st2k_appkit::uia`).
pub(super) use st2k_appkit::uia::WM_UIA_JOB;
    ...
    F: FnOnce(HWND, &mut Shot) -> T + Send + 'static,
{
    ...
    // re-entrancy guard.
    st2k_appkit::uia::on_ui_thread(hwnd_of(hwnd_raw), move |hwnd| {
...
pub(super) unsafe fn run_job(hwnd: HWND, lparam: LPARAM) -> LRESULT {
    if reentrant() {
        ...
        st2k_appkit::uia::discard_job(lparam);
        return LRESULT(0);
    }
    st2k_appkit::uia::run_job(hwnd, lparam)
}
