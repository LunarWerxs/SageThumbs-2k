...
pub(super) unsafe fn draw_dim_badge(hdc: HDC, s: &Shot, sel: RECT) {
    ...
    let padx = st2k_appkit::win::dpi_scale_dpi(8, dpi);
    let pady = st2k_appkit::win::dpi_scale_dpi(3, dpi);
    let gap = st2k_appkit::win::dpi_scale_dpi(6, dpi);
...
fn size_readout(tool: Tool, thickness: i32, text_size: i32) -> String {
    if tool == Tool::Text {
        st2k_appkit::win::t("shot_hint_size_text").replace("{n}", &text_size.to_string())
    } else {
        st2k_appkit::win::t("shot_hint_size_generic").replace("{n}", &thickness.to_string())
    }
}
...
fn snap_suffix(tool: Tool, forced: Option<bool>) -> &'static str {
    ...
    match forced {
        Some(true) => st2k_appkit::win::t("shot_hint_snap_on"),
        Some(false) => st2k_appkit::win::t("shot_hint_snap_off"),
        None => st2k_appkit::win::t("shot_hint_snap_shift"),
    }
}
...
fn format_active_hint(tool: &str, size: &str, color_hex: &str, snap: &str) -> String {
    st2k_appkit::win::t("shot_hint_active")
        .replace("{tool}", tool)
        .replace("{size}", size)
...
pub(super) unsafe fn draw_hint(hdc: HDC, s: &Shot) {
    ...
    let txt = if s.sel.is_none() {
        match (s.ocr_mode, s.sel_dragging) {
            ...
            (true, false) => st2k_appkit::win::t("shot_hint_ocr_drag").to_string(),
            (true, true) => st2k_appkit::win::t("shot_hint_ocr_release").to_string(),
            (false, false) => st2k_appkit::win::t("shot_hint_drag").to_string(),
            (false, true) => st2k_appkit::win::t("shot_hint_release").to_string(),
        }
    } else {
    ...
    let bar_w = s.vw.min(st2k_appkit::win::dpi_scale_dpi(1080, dpi));
    let bar_h = st2k_appkit::win::dpi_scale_dpi(26, dpi);
    let gap = st2k_appkit::win::dpi_scale_dpi(6, dpi); // gap above the selection
    let inset = st2k_appkit::win::dpi_scale_dpi(4, dpi); // inset when there's no room above
    ...
    SetTextColor(hdc, rgb(235, 235, 235));
    let w = wide(&txt);
    let tx = st2k_appkit::win::dpi_scale_dpi(10, dpi); // text left padding
    let ty = st2k_appkit::win::dpi_scale_dpi(5, dpi); // text top padding
...
mod tests {
    ...
    fn size_readout_reads_the_locale_table() {
        assert_eq!(
            ...
            st2k_appkit::win::t("shot_hint_size_text").replace("{n}", "32"),
            "the Text tool must report the FONT size, not the line thickness"
        );
        assert_eq!(
            size_readout(Tool::Rect, 4, 32),
            st2k_appkit::win::t("shot_hint_size_generic").replace("{n}", "4"),
    ...
    fn snap_suffix_only_applies_to_line_and_arrow_and_is_localized() {
        ...
        for tool in [Tool::Line, Tool::Arrow] {
            assert_eq!(
                ...
                st2k_appkit::win::t("shot_hint_snap_shift")
            );
            assert_eq!(
                snap_suffix(tool, Some(true)),
                st2k_appkit::win::t("shot_hint_snap_on")
            );
            assert_eq!(
                snap_suffix(tool, Some(false)),
                st2k_appkit::win::t("shot_hint_snap_off")
            );
        }
    }
