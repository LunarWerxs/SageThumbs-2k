...
use hosts::*;
mod wire;
pub use hosts::open_hosts_config;
use wire::*;
...
use st2k_appkit::win::{set_clipboard_text, t, wide, SS_CENTER, SS_CENTERIMAGE};
...
pub unsafe fn with_busy_pill<T: Send + 'static>(
    text: &str,
    ...
    if let Some(p) = pill {
        SendMessageW(
            ...
            Some(windows::Win32::Foundation::WPARAM(
                st2k_appkit::win::gui_font().0 as usize,
            )),
...
pub unsafe fn run_upload(path: &str) {
...
pub unsafe fn run_upload_keep(list_path: &str, url_to: Option<&str>) {
