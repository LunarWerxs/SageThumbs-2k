...
unsafe extern "system" fn win_event_proc(
    ...
    let cls = st2k_appkit::explorer_selection::class_name(hwnd);
    if st2k_base::settings::served_window_kind(&cls).is_none() {
        return;
    }
    let _ = PostMessageW(
...
pub(super) unsafe fn warning_for(hwnd: HWND) -> Option<&'static str> {
    ...
    let cls = st2k_appkit::explorer_selection::class_name(hwnd);
    let kind = st2k_base::settings::served_window_kind(&cls)?;
    ...
    if !st2k_base::host::process_is_elevated(pid) {
