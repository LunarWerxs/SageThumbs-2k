...
unsafe fn foreground_qualifies(fg: HWND) -> bool {
    let cls = st2k_appkit::explorer_selection::class_name(fg);
    ...
    match cls.as_str() {
        ...
        "#32770" => st2k_appkit::dialog_hook::is_file_dialog(fg),
        ...
        _ => {
            st2k_appkit::explorer_selection::is_everything_class(&cls)
                && (st2k_appkit::explorer_selection::everything_focus_window(fg).is_some()
                    || st2k_appkit::explorer_selection::everything_result_list(fg).is_some())
        }
    }
}
...
unsafe fn is_typing(fg: HWND) -> bool {
    ...
    if GetGUIThreadInfo(tid, &mut gti).is_ok() {
        ...
        if !gti.hwndFocus.0.is_null()
            && st2k_appkit::explorer_selection::class_name(gti.hwndFocus)
