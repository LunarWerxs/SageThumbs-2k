...
//! [`st2k_appkit::explorer_selection`]). The helper runs one action, then exits.
...
pub fn action_label(key: &str) -> &'static str {
    st2k_appkit::win::t(key)
}
...
pub unsafe fn run_hotkey_action(hinst: HINSTANCE) {
    ...
    match kind {
        ...
        Kind::OpenSettings => {
            let _ = st2k_appkit::win::spawn_self(&[]);
        }
        ...
        Kind::Preview => {
            let _ = st2k_appkit::win::spawn_self(&["--preview"]);
        }
...
unsafe fn run_on_selection(action: VerbAction, images_only: bool) {
    use st2k_appkit::explorer_selection::SelectionOutcome;
    let paths = match st2k_appkit::explorer_selection::selection_or_pick(images_only) {
        ...
        SelectionOutcome::VirtualOnly => {
            ...
            st2k_appkit::win::message_box(
                ...
                st2k_appkit::win::t("hotkey_selection_virtual"),
                "SageThumbs 2K",
            );
            return;
        }
