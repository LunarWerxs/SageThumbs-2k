...
//! [`st2k_codecs::strip::isobmff`]) and TIFF (whose IFD0 is walked and rebuilt without its
...
fn read_heif_metadata(bytes: &[u8]) -> Carried {
    ...
    out.exif = exif;
    out.xmp = xmp;
    out.icc = st2k_codecs::isobmff::color_profile(bytes);
    out
}
...
fn read_isobmff(bytes: &[u8]) -> (Option<Vec<u8>>, Option<Vec<u8>>) {
    let mut exif = None;
    let mut xmp = None;
    for item in st2k_codecs::isobmff::items(bytes) {
...
fn item_bytes<'a>(bytes: &'a [u8], item: &st2k_codecs::isobmff::Item) -> Option<&'a [u8]> {
