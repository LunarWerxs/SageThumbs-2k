...
pub const URL_PARENT: &str = "https://lunarwerx.com";
...
pub const URL_PRODUCT: &str = "https://github.com/LunarWerxs/SageThumbs-2k";
pub const URL_GITHUB: &str = "https://github.com/LunarWerxs/SageThumbs-2k";
...
pub fn top_down_bgra_bmi(w: i32, h: i32) -> BITMAPINFO {
    BITMAPINFO {
...
pub fn command_parts(wparam: WPARAM) -> (i32, u32) {
    (
...
pub fn cursor_monitor_metrics() -> (i32, windows::Win32::Foundation::RECT) {
...
pub unsafe fn register_app_class(class: PCWSTR, wndproc: WNDPROC, hinst: HINSTANCE) {
    let wc = WNDCLASSW {
        lpfnWndProc: wndproc,
        hInstance: hinst,
        lpszClassName: class,
...
pub unsafe fn create_tooltip_window(parent: HWND, hinst: HINSTANCE) -> Option<HWND> {
    CreateWindowExW(
        WINDOW_EX_STYLE(0),
        w!("tooltips_class32"),
        PCWSTR::null(),
...
/// uses, 26px tall.
pub unsafe fn measure_menu_item(hwnd: HWND, m: &mut MEASUREITEMSTRUCT, label: &str) {
...
pub unsafe fn draw_menu_item(d: &DRAWITEMSTRUCT, label: &str) {
...
pub unsafe fn pump_until_quit(hwnd: HWND) {
    let mut msg = MSG::default();
...
pub const STATIC: PCWSTR = w!("STATIC");
pub const BUTTON: PCWSTR = w!("BUTTON");
pub const EDIT: PCWSTR = w!("EDIT");
pub const COMBOBOX: PCWSTR = w!("COMBOBOX");
pub const SYSLINK: PCWSTR = w!("SysLink");
...
pub const MARGIN: i32 = 16; // left edge of group labels
pub const INDENT: i32 = 26; // left edge of indented (in-group) controls
pub const LABEL_W: i32 = 190; // label column width (settings limits rows)
pub const EDIT_X: i32 = 224; // left edge of the edit/value column (settings)
pub const BTN_H: i32 = 28; // standard pushbutton height
...
pub mod winshim {
    ...
    pub const SS_REALSIZECONTROL: u32 = 0x0000_0040;
    ...
    pub const BM_GETCHECK_MSG: u32 = 0x00F0;
    pub const BM_SETCHECK_MSG: u32 = 0x00F1;
    ...
    pub const CHECKED: u32 = 0x2000;
    pub const UNCHECKED: u32 = 0x1000;
}
pub use winshim::*;

pub const fn make_lparam(low: i32, high: i32) -> isize {
...
pub fn read_listfile(path: &str) -> Vec<String> {
...
pub fn wstr_to_string(w: &[u16]) -> String {
...
pub unsafe fn open_notify_link(link: *const NMLINK) {
...
pub unsafe fn load_art(default_png: &[u8], override_name: &str, w: u32, h: u32) -> Option<HBITMAP> {
...
pub unsafe fn app_icon() -> Option<HICON> {
...
pub fn spawn_self(args: &[&str]) -> bool {
...
pub unsafe fn set_static_bitmap(ctl: HWND, hbmp: HBITMAP) {
...
pub unsafe fn check(hwnd: HWND, id: i32, on: bool) {
...
pub unsafe fn checked(hwnd: HWND, id: i32) -> bool {
...
pub unsafe fn combo_sel(hwnd: HWND, id: i32) -> usize {
    GetDlgItem(Some(hwnd), id)
        .map(|c| SendMessageW(c, CB_GETCURSEL, None, None).0.max(0) as usize)
        .unwrap_or(0)
}

pub unsafe fn set_edit_text(hwnd: HWND, id: i32, text: &str) {
    if let Ok(h) = GetDlgItem(Some(hwnd), id) {
        let w = wide(text);
...
pub unsafe fn get_edit_text(hwnd: HWND, id: i32) -> String {
