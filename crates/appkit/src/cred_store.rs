...
pub use st2k_base::settings::{is_credential_root_value, is_credential_subkey};
...
pub fn save_refresh_token(token: &str) -> bool {
...
pub fn save_identity(sub: &str, email: &str, name: &str, picture: &str) {
    if settings::portable() {
        for (key, value) in [
            (V_SUB, sub),
            (V_EMAIL, email),
