...
pub const WM_APP_SPONSORS: u32 = 0x8000 + 7; // WM_APP + 7
pub const TIMER_BANNER: usize = 1; // animates the current image's GIF frames
pub const TIMER_ROTATE: usize = 2; // advances to the next sponsor / image

/// One decoded piece of art: a single HBITMAP for a still image, or many for an
/// animated GIF (with the inter-frame `delay_ms`).
pub struct SponsorImage {
    pub frames: Vec<isize>,   // one HBITMAP handle per frame
...
pub struct SponsorRotator {
    pub sponsors: Vec<Sponsor>,
    pub cur: usize,   // current sponsor
    pub img: usize,   // current image within the sponsor
    pub frame: usize, // current GIF frame within the image
    pub rotate_ms: u32,
    random: bool,
...
pub unsafe fn show_current_image(hwnd: HWND, banner: HWND, r: &SponsorRotator, free_prev: bool) {
...
pub unsafe fn drop_sponsor_rotator(ptr: *mut SponsorRotator) {
    if ptr.is_null() {
        return;
    }
