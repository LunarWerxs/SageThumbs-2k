...
pub use everything::{everything_focus_window, everything_result_list, is_everything_class};
use picker::*;
...
pub unsafe fn selection_or_pick(images_only: bool) -> SelectionOutcome {
...
pub unsafe fn preview_target() -> PreviewTarget {
    let _com = st2k_base::parallel::ComGuard::sta();
...
pub unsafe fn resolve_explicit(path: &str) -> String {
    let _com = st2k_base::parallel::ComGuard::sta();
    resolve_lnk(path)
}
