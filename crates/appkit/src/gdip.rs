...
pub unsafe fn with_aa(hdc: HDC, f: impl FnOnce(*mut GpGraphics)) {
...
pub unsafe fn pen(color: COLORREF, w: i32) -> *mut GpPen {
...
pub unsafe fn pen_round(color: COLORREF, w: i32) -> *mut GpPen {
    let p = pen(color, w);
    if !p.is_null() {
...
pub unsafe fn drop_pen(p: *mut GpPen) {
    let _ = GdipDeletePen(p);
}

/// A solid fill brush of `color`. Free with [`drop_brush`].
pub unsafe fn brush(color: COLORREF) -> *mut GpBrush {
    let mut b: *mut GpSolidFill = core::ptr::null_mut();
    let _ = GdipCreateSolidFill(argb(color), &mut b);
    b as *mut GpBrush
}
pub unsafe fn drop_brush(b: *mut GpBrush) {
    let _ = GdipDeleteBrush(b);
}

// ---- drawing (all take GDI+ integer coordinates; x,y = top-left, w,h = extent) ----

pub unsafe fn line(g: *mut GpGraphics, p: *mut GpPen, x1: i32, y1: i32, x2: i32, y2: i32) {
    let _ = GdipDrawLineI(g, p, x1, y1, x2, y2);
}
pub unsafe fn rect(g: *mut GpGraphics, p: *mut GpPen, x: i32, y: i32, w: i32, h: i32) {
    let _ = GdipDrawRectangleI(g, p, x, y, w, h);
}
pub unsafe fn ellipse(g: *mut GpGraphics, p: *mut GpPen, x: i32, y: i32, w: i32, h: i32) {
    let _ = GdipDrawEllipseI(g, p, x, y, w, h);
}
pub unsafe fn fill_rect(g: *mut GpGraphics, b: *mut GpBrush, x: i32, y: i32, w: i32, h: i32) {
    let _ = GdipFillRectangleI(g, b, x, y, w, h);
}
pub unsafe fn fill_ellipse(g: *mut GpGraphics, b: *mut GpBrush, x: i32, y: i32, w: i32, h: i32) {
    let _ = GdipFillEllipseI(g, b, x, y, w, h);
}
/// Connected line segments (a polyline) through `pts`.
pub unsafe fn polyline(g: *mut GpGraphics, p: *mut GpPen, pts: &[(i32, i32)]) {
    if pts.len() < 2 {
        return;
    }
...
pub unsafe fn ocr_glyph(hdc: HDC, r: windows::Win32::Foundation::RECT, ink: COLORREF, em: i32) {
