...
pub const WM_UIA_JOB: u32 = WM_APP + 60;
...
pub fn on_ui_thread<T, F>(hwnd: HWND, f: F) -> Option<T>
where
    T: Send + 'static,
...
pub unsafe fn run_job(hwnd: HWND, lparam: LPARAM) -> LRESULT {
...
pub unsafe fn discard_job(lparam: LPARAM) {
    let ptr = lparam.0 as *mut Arc<dyn Job>;
    if !ptr.is_null() {
...
/// invalidate.
pub unsafe fn on_destroy(hwnd: HWND) {
...
/// nobody.
pub unsafe fn raise_selection_changed(item_hwnd: HWND, ops: &'static ItemOps) {
