...
pub(super) fn virtual_item() -> InfoCard {
    InfoCard {
        name: st2k_appkit::win::t("ic_virtual_title").to_string(),
        detail: st2k_appkit::win::t("ic_virtual_detail").to_string(),
        icon: None,
    }
}
...
pub(super) unsafe fn paint(
    ...
    let name_font = st2k_appkit::win::gui_font_sized(hwnd, 15, 600);
    ...
    let det_font = st2k_appkit::win::gui_font_sized(hwnd, 12, 400);
...
unsafe fn shell_icon(path: &str) -> Option<HICON> {
    let wide = st2k_appkit::win::wide(path);
