...
impl FindState {
    ...
    fn counter(&self) -> String {
        ...
        if self.hits.is_empty() {
            return st2k_appkit::win::t("find_no_results").to_string();
        }
        format!(
            "{}/{}",
...
pub(in crate::preview) unsafe fn bar_rect(hwnd: HWND) -> RECT {
    ...
    let cap = st2k_appkit::win::dpi_scale(hwnd, super::window::CAPTION_H);
    let h = st2k_appkit::win::dpi_scale(hwnd, FIND_H);
...
pub(in crate::preview) unsafe fn paint(hwnd: HWND, hdc: HDC, text: u32, subtle: u32) {
    ...
    let f = st.find.borrow();
    let font = st2k_appkit::win::gui_font_for(hwnd);
