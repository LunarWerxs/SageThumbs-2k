...
pub(super) unsafe fn paint_lines(
    ...
    let colors = Colors {
        plain: fg,
        comment: st2k_appkit::dark::CODE_COMMENT().0,
        string: st2k_appkit::dark::CODE_STRING().0,
        num: st2k_appkit::dark::CODE_NUMBER().0,
        keyword: st2k_appkit::dark::CODE_KEYWORD().0,
    };
    let sp = spec(lang);
    ...
    let gutter_fg = st2k_appkit::dark::HEADER_TEXT().0;
    let sel_bg = st2k_appkit::dark::SEL_BG().0;
