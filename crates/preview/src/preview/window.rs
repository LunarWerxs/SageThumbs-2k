...
pub(super) unsafe fn invalidate_caption(hwnd: HWND) {
    ...
    r.bottom = st2k_appkit::win::dpi_scale(hwnd, CAPTION_H);
...
thread_local! {
    ...
    /// [`st2k_appkit::gdip::with_aa`] silently draws NOTHING (an empty button, which is exactly
...
pub(super) unsafe fn create_viewer(
    ...
    if dark {
        st2k_appkit::dark::dark_titlebar(hwnd);
    }
...
pub(super) unsafe fn set_title(hwnd: HWND) {
    ...
    let w = st2k_appkit::win::wide(&name);
