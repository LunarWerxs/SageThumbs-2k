...
pub(super) unsafe fn video_rect(hwnd: HWND) -> RECT {
    ...
    r.bottom -= st2k_appkit::win::dpi_scale(hwnd, SCRUB_H);
    r
}
...
pub(super) unsafe fn scrub_rect(hwnd: HWND) -> RECT {
    ...
    let h = st2k_appkit::win::dpi_scale(hwnd, SCRUB_H);
    RECT {
        left: c.left,
        top: c.bottom - h,
        right: c.right,
...
pub(super) unsafe fn draw_scrub_strip(
    ...
    let sc = |val: i32| st2k_appkit::win::dpi_scale(hwnd, val);
    let bg = CreateSolidBrush(COLORREF(st2k_appkit::dark::DARK_BG().0));
    FillRect(hdc, sr, bg);
    ...
    let border = st2k_appkit::dark::BORDER().0;
...
unsafe fn draw_right_controls(
    ...
    let sp = v.speed();
    let f2 = st2k_appkit::win::gui_font_for(hwnd);
