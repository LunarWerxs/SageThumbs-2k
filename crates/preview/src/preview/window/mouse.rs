...
unsafe fn sel_drag_move(hwnd: HWND, st: &ViewerState, x: i32, y: i32) {
    ...
    if overshoot != 0 {
        let step_cap = st2k_appkit::win::dpi_scale(hwnd, 40);
...
unsafe fn setcursor_client(hwnd: HWND, st: &ViewerState) -> Option<PCWSTR> {
    ...
    if selection::selectable(st.kind.get())
        && pt.y >= st2k_appkit::win::dpi_scale(hwnd, CAPTION_H)
...
pub(super) unsafe fn on_mousewheel(hwnd: HWND, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    match st.kind.get() {
        ...
        ContentKind::Image if crate::preview::pdfview::active(hwnd) => {
            // Three lines a notch.
            let step = pdf_wheel_step(delta, st2k_appkit::win::dpi_scale(hwnd, 54));
