...
unsafe fn on_btn_save_page(hwnd: HWND, st: &ViewerState, path: Option<String>) {
    ...
    let Some(dest) = st2k_appkit::win::pick_save_png(hwnd, &dir, &suggested) else {
...
pub(in crate::preview) unsafe fn toggle_theme(hwnd: HWND) {
    ...
    st2k_appkit::dark::set_theme_override(Some(dark));
    st2k_appkit::dark::titlebar_theme(hwnd, dark);
