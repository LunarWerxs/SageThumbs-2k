...
pub(in crate::preview) unsafe fn strip_width(hwnd: HWND) -> i32 {
    ...
    if r.right < st2k_appkit::win::dpi_scale(hwnd, STRIP_MIN_CLIENT) {
        return 0;
    }
    st2k_appkit::win::dpi_scale(hwnd, STRIP_W)
}
...
pub(super) fn strip_pitch(hwnd: HWND, thumb_h: i32) -> i32 {
    thumb_h + st2k_appkit::win::dpi_scale(hwnd, 22)
}
...
pub(in crate::preview) unsafe fn paint_strip(hwnd: HWND, hdc: HDC, bg: u32, sheet: u32) -> bool {
    ...
    let pad = st2k_appkit::win::dpi_scale(hwnd, 8);
    ...
    let fonts = st2k_appkit::win::gui_font_for(hwnd);
    let oldf = SelectObject(hdc, fonts.into());
    SetBkMode(hdc, TRANSPARENT);
    SetTextColor(
        hdc,
        windows::Win32::Foundation::COLORREF(st2k_appkit::dark::HEADER_TEXT().0),
    );
    ...
    for slotn in 0..=visible {
        ...
        if page == cur {
            ...
            let f = st2k_appkit::win::dpi_scale(hwnd, 2);
            let outer = RECT {
                left: cell.left - f,
                top: cell.top - f,
                right: cell.right + f,
        ...
        let label = st2k_appkit::win::wide(&format!("{}", page + 1));
        let mut lr = RECT {
            left: cell.left,
            top: cell.bottom,
            right: cell.right,
            bottom: cell.bottom + st2k_appkit::win::dpi_scale(hwnd, 18),
        };
