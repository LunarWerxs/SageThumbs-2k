...
//! This library uses no foreign code. The SageThumbs vendored build contains
//! one narrowly scoped unsafe implementation for leak-free, loader-zeroed DWA
//! transfer-table storage; see `SAGETHUMBS-PATCH.md`.
//!
...
// Keep unsafe code denied crate-wide while allowing the DWA transfer-table
// storage implementation to opt in on three individually documented items.
#![deny(unsafe_code)]
#![warn(missing_docs)]
